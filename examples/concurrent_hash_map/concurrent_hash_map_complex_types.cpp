...
using hashmap_type = concurrent_hash_map<p<int>, persistent_ptr<string>>;
...
struct root {
	persistent_ptr<hashmap_type> pptr = nullptr;
};
		...
		for (int i = 0; i < THREADS_NUM / 3; ++i) {
			threads.emplace_back([&]() {
				for (int i = 0; i < 10 * THREADS_NUM; ++i) {
					persistent_ptr<string> tmp_string;
					transaction::run(pop, [&] {
						/* note that allocation in transaction is made by
						 * make_persistent */
						tmp_string = make_persistent<string>(std::to_string(i));
					});

					map.insert(
						hashmap_type::value_type(i, tmp_string ));
				}
			});
		}
		...
		for (int i = 0; i < THREADS_NUM / 3; ++i) {
			threads.emplace_back([&]() {
				for (int i = 0; i < 10 * THREADS_NUM; ++i) {
					...
					if (res) {
						assert(acc->first == i);
						persistent_ptr<string> v = acc->second;
