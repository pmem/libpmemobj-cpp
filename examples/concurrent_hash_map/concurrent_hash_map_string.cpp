...
/*
 * concurrent_hash_map_string.cpp -- C++ documentation snippets.
 */

//! [concurrent_hash_map_string_ex]
#include <iostream>
 ...
 * explicitely use transacions if strings are stored in concurrent_hash_map */
...
/* In this example and we only need place concurrent_hash_map in the pool. Hence
 * we will correlate memory pool root object with single instance of persistent
 ...
 * pmempool create obj --layout="concurrent_hash_map_string" --size 1G
 * path_to_a_pool
 */
int
	...
	try {
		...
		try {
			pop = pmem::obj::pool<root>::open(
				path, "concurrent_hash_map_string");
		...
		for (int i = 0; i < THREADS_NUM / 3; ++i) {
			std::string tmp_string = std::to_string(i);
			/* Implicit conversion from std::string to
			 * pmem::obj::string. */
			map.insert_or_assign(i, tmp_string);
		}

		/* Check if given key is in the hashmap. For the time of an
		 * accessor life, the read lock is taken on the item. */
