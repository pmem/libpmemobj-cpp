...
insert_elements_kv(pmem::obj::pool<root> pop)
{
	...
	pmem::obj::transaction::run(pop, [&] {
		...
		try {
			...
			/* The capacity might not be enough, use assing_val
			 * instead. */
		}
		...
		/* it is still valid */
		assert(pmem::obj::string_view(it->value()).compare("some long string") == 0);

		/* If reallocation happend on assing_val, it2 will be
		 * invalidated */
		(void) it2;
