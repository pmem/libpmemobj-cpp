...
public:
	...
	skip_list_node(size_type levels) : height_(levels)
	{
		...
		for (size_type lev = 0; lev < height_; ++lev) {
			VALGRIND_HG_DISABLE_CHECKING(
				&get_next(lev), sizeof(atomic_node_pointer));
		}
#endif
	}
	...
	{
		assert(level < height());
		get_next(level).store(next, std::memory_order_release);
	}
	...
	{
		set_next(level, next);
		pop.persist(&get_next(level), sizeof(atomic_node_pointer));
	}
		  ...
		  size_type h)
	{
		...
		auto *nexts = get_nexts();
		pop.persist(nexts, sizeof(atomic_node_pointer) * h);
	}
