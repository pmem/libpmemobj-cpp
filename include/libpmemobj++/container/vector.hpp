...
/**
 * Destructor.
 * Note that free_data may throw an transaction_free_error
 * when freeing underlying segments failed. It is recommended
 * to call free_data manually before object destruction,
 * in other case application can be terminated on failure.
 */
template <typename T>
vector<T>::~vector()
{
