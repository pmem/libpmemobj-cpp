...
private:
	...
	internal_try_emplace(K &&key, Args &&... args)
	{
		...
		std::pair<iterator, bool> insert_result = internal_insert_node(
			key, height, tls_entry, std::piecewise_construct,
	...
	internal_emplace(Args &&... args)
	{
		...
		std::pair<iterator, bool> insert_result =
			internal_insert_node(get_key(n), n->height(), tls_entry,
					     std::forward<Args>(args)...);
	...
	internal_insert(const K &key, Args &&... args)
	{
		...
		std::pair<iterator, bool> insert_result = internal_insert_node(
			key, height, tls_entry, std::forward<Args>(args)...);
	...
	internal_insert_node(const K &key, size_type height,
			     tls_entry_type &tls_entry, Args &&... args)
	{
		...
		do {
			...
			if (next && !allow_multimapping &&
				...
				return std::pair<iterator, bool>(
				  ...
				  prev_nodes, next_nodes, height, tls_entry,
	...
	template <typename... Args>
	persistent_node_ptr
	create_node(size_type levels, const next_array_type &nexts,
		    Args &&... args)
	{
		assert(pmemobj_tx_stage() == TX_STAGE_WORK);

		persistent_node_ptr node =
			creates_dummy_node(levels, nexts.data());
