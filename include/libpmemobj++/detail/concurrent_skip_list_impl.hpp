...
public:
	...
	skip_list_node(size_type levels, node_pointer *new_nexts)
	    : height_(levels)
	{
		for (size_type lev = 0; lev < height_; ++lev)
...
public:
	      ...
	      _rnd_generator(other._rnd_generator)
	{
		...
		init();
		internal_copy(other);
		assert(_size.get_ro() == other._size.get_ro());
	}
	...
	runtime_initialize()
	{
		auto actual_size = std::distance(this->begin(), this->end());
	...
	size() const
	{
		return _size.get_ro().load(std::memory_order_relaxed);
	}
	...
	swap(concurrent_skip_list &other)
	{
		...
		obj::transaction::run(pop, [&] {
			...
			size_type tmp = _size.get_ro();
			_size.get_rw().store(other._size.get_ro());
			other._size.get_rw().store(tmp);
		});
	}
...
private:
	...
	{
		_size.get_rw().store(actual_size, std::memory_order_relaxed);
		obj::pool_base pop = get_pool_base();
		pop.persist(_size);
	}
			...
			Args &&... args)
	{
		...
		/* Make sure that new_node is a pointer from TLS*/
		assert(&new_node == &get_tls_node_ptr());
		...
		VALGRIND_SET_CLEAN(&new_node, sizeof(new_node));
#endif
	...
	template <typename... Args>
	persistent_node_ptr
	create_node(size_type levels, persistent_node_ptr *nexts,
		    Args &&... args)
	{
		assert(pmemobj_tx_stage() == TX_STAGE_WORK);

		persistent_node_ptr node = creates_dummy_node(levels, nexts);
	...
	struct alignas(64) tls_entry_type {
	...
	tls_restore()
	{
		new_node_tls.initialize([this](tls_entry_type &tls_entry) {
			persistent_node_ptr &node = tls_entry.ptr;
			if (node) {
				complete_insert(node);
			}
			if (node) {
				delete_node(node);
			}
	...
	void
	complete_insert(persistent_node_ptr &node)
	{
		prev_array_type prev_nodes;
		next_array_type next_nodes;
		node_ptr n = node.get(pool_uuid);
		...
		if (node == next_nodes[0]) {
			...
			for (size_type level = 0; level < height; ++level) {
				...
				if (prev_nodes[level]->next(level) != node) {
					...
					assert(n->next(level) ==
					       next_nodes[level]);
					prev_nodes[level]->set_next(pop, level,
								    node);
				}
			}
			...
			node = nullptr;
#if LIBPMEMOBJ_CPP_VG_PMEMCHECK_ENABLED
			VALGRIND_SET_CLEAN(&node, sizeof(node));
#endif
		} else {
			internal_insert_node(key, height, node);
		}
	}
	...
	enumerable_thread_specific<tls_entry_type> new_node_tls;

	obj::p<std::atomic<size_type>> _size;
}; /* class concurrent_skip_list */
