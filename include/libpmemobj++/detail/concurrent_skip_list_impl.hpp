...
private:
	...
	{
		auto &tls_entry = tls_data.local();
		persistent_node_ptr &new_node = tls_entry.ptr;
			...
			Args &&... args)
	{
		...
		if (new_node) {
			...
			assert(tls_data.local().ptr != nullptr);
			n = new_node.get(pool_uuid);
		}
		...
		new_node = nullptr;
#if LIBPMEMOBJ_CPP_VG_PMEMCHECK_ENABLED
		VALGRIND_PMC_REMOVE_PMEM_MAPPING(&new_node, sizeof(new_node));
#endif
	...
	complete_insert(persistent_node_ptr &node)
	{
		...
		if (node == next_nodes[0]) {
			...
			node = nullptr;
#if LIBPMEMOBJ_CPP_VG_PMEMCHECK_ENABLED
			VALGRIND_PMC_REMOVE_PMEM_MAPPING(&node, sizeof(node));
#endif
		} else {
