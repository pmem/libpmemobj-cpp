...
protected:
	 ...
	 */
	difference_type
	distance_between_self(const self_relative_ptr_base_impl &ptr) const
		noexcept
	{
	...
	pointer_to_offset(const self_relative_ptr_base_impl &ptr) const noexcept
	{
		...
		*/
		...
		--mask;
		difference_type offset_between_self =
			this->distance_between_self(ptr);
		offset_between_self &= static_cast<difference_type>(mask);
		return ptr.offset + offset_between_self;
	}
...
public:
	...
	static difference_type
	pointer_to_offset(const access_type &obj, void *ptr)
	{
		return obj.pointer_to_offset(ptr);
	}
	...
	{
		// local obj for transform offset to pointer
		access_type temp{offset};
		auto ptr = temp.to_byte_pointer();
		if (ptr != nullptr)
			ptr += temp.distance_between_self(obj);
		return reinterpret_cast<PointerType *>(ptr);
	}
