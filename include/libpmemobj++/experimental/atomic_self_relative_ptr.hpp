...
#define __VALGRIND_ANNOTATE_HAPPENS_BEFORE(order, ptr)                         \
...
#define __VALGRIND_ANNOTATE_HAPPENS_AFTER(order, ptr)                          \
 ...
 * Equal std::atomic<T*> but return types self_relative_ptr
 */
template <typename T>
...
public:
	...
	{
		pmem::detail::conditional_add_to_tx(this);
		auto pointer = desired.to_void_pointer();
		auto offset = accessor::pointer_to_offset(ptr, pointer);
		__VALGRIND_ANNOTATE_HAPPENS_BEFORE(order, &ptr);
		      ...
		      std::memory_order_seq_cst) volatile noexcept
	{
		...
		auto pointer = desired.to_void_pointer();
		auto offset = accessor::pointer_to_offset(
			const_cast<ptr_type &>(ptr), pointer);
	...
	load(std::memory_order order = std::memory_order_seq_cst) const noexcept
	{
		...
		__VALGRIND_ANNOTATE_HAPPENS_AFTER(order, &ptr);
		...
		volatile noexcept
	{
		...
		auto pointer = accessor::offset_to_pointer<T>(
			offset, const_cast<ptr_type &>(ptr));
		return value_type{pointer};
	}
	...
	{
		pmem::detail::conditional_add_to_tx(this);
		auto desired_ptr = desired.to_void_pointer();
		auto new_offset = accessor::pointer_to_offset(ptr, desired_ptr);
		auto old_offset =
	...
	{
		pmem::detail::conditional_add_to_tx(this);
		auto expected_offset = accessor::pointer_to_offset(
			ptr, expected.to_void_pointer());
		auto desired_offset = accessor::pointer_to_offset(
			ptr, desired.to_void_pointer());

		bool result = accessor::get_offset(ptr).compare_exchange_weak(
			expected_offset, desired_offset, success, failure);
		if (!result)
			expected = accessor::offset_to_pointer(expected_offset,
							       ptr);
		return result;
	}
	...
	value_type
	operator=(value_type desired) volatile noexcept
	{
		store(desired);
		return desired;
	}
	...
	value_type
	operator+=(difference_type diff) volatile noexcept
	{
		return this->fetch_add(diff) + diff;
	}
