...
/**
 * Takes a “snapshot” of given elements std::atomic<self_relative_ptr<T>> and
...
template <typename T>
void
snapshot(const std::atomic<obj::experimental::self_relative_ptr<T>> *addr,
	 size_t num = 1)
{
	using snapshot_type =
		obj::experimental::self_relative_ptr_base::offset_type;
	static_assert(sizeof(snapshot_type) == sizeof(*addr),
		      "std::atomic<self_relative_ptr> should be the same size");
	obj::transaction::snapshot<snapshot_type>(
		reinterpret_cast<const snapshot_type *>(addr), num);
}
