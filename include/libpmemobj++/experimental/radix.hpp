...
radix_tree<Value>::find(obj::string_view key)
{
	...
	if (key.compare(n.get_leaf()->key()) != 0)
		return end();
...
{
	auto k_acc = key_accessor();
	return {k_acc.get().data(), k_acc.get().size()};
}
...
radix_tree<Value>::leaf::value()
{
	return value_accessor(); // XXX: get rid of that, just expose
				 // const view of value and only allow
...
radix_tree<Value>::leaf::key_accessor()
{
	...
	return key_type::accessor({k_memory, k_memory + data.first.size()},
				  data.first);
}
...
radix_tree<Value>::leaf::value_accessor()
{
	...
	auto capacity = pmemobj_alloc_usable_size(pmemobj_oid(this)) -
		sizeof(leaf) - key().size();
	return key_type::accessor({v_memory, v_memory + capacity}, data.second);
}
