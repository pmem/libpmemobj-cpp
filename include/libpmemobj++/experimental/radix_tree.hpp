 ...
 * depence on operations currently performed by other threads.
 *
 * Garbage is not automatically collected on move/copy ctor/assignment.
 *
 * @throw pmem::transaction_error when snapshotting failed.
...
/**
 * Checks if iterator points to element larger (or equal) than key.
 */
template <typename Key, typename Value, typename BytesView>
template <bool Lower, typename K>
bool
...
/**
 * Checks if any node in the `path` was modified.
 */
template <typename Key, typename Value, typename BytesView>
bool
radix_tree<Key, Value, BytesView>::validate_path(const path_type &path) const
{
	/* Path cannot change in singlethreaded workload */
...
radix_tree<Key, Value, BytesView>::internal_bound(const K &k) const
{
	...
	while (true) {
		...
		if (!n) {
			 ...
			 * target node must be the smallest leaf within *slot
			...
			/* The target node must be within *slot subtree. The
		...
		/* If some node on the was modified the calculated result might
		 * not be correct. */
