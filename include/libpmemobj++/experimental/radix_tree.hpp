...
class radix_tree {
	...
	using v_reference = typename std::conditional<
		std::is_same<Value, inline_string>::value,
		inline_string_reference<false>, Value &>::type;

	using const_v_reference = typename std::conditional<
		std::is_same<Value, inline_string>::value,
		inline_string_reference<true>, const Value &>::type;
...
public:
	...
	using const_key_reference = typename std::conditional<
		...
		const key_type &>::type;
	using reference = std::pair<const_key_reference, v_reference>;
	using const_reference =
		std::pair<const_key_reference, const_v_reference>;
...
public:
	...
	using reference =
		typename std::conditional<IsConst,
					  typename radix_tree::const_reference,
					  typename radix_tree::reference>::type;
	using pointer = reference *;
	using iterator_category = std::bidirectional_iterator_tag;

private:
	using key_reference = decltype(std::declval<reference>().first);
	using value_reference = decltype(std::declval<reference>().second);

	struct pointer_proxy {
		pointer_proxy(reference r);
		pointer operator->();
...
template <typename Key, typename Value, typename BytesView>
template <bool IsConst>
radix_tree<Key, Value, BytesView>::radix_tree_iterator<IsConst>::pointer_proxy::
	pointer_proxy(
		typename radix_tree<Key, Value, BytesView>::
			template radix_tree_iterator<IsConst>::reference r)
    : r(r)
{
}
...
typename radix_tree<Key, Value,
	...
	radix_tree<Key, Value, BytesView>::radix_tree_iterator<
		IsConst>::pointer_proxy::operator->()
{
	return &r;
}
...
radix_tree<Key, Value, BytesView>::lower_bound(const_key_reference k) const
{
	...
	if (!n) {
		return const_iterator(
			next_leaf<node::direction::Forward>(
				prev->make_iterator(child_slot), prev),
			&root);
	}

	assert(n.is_leaf());

	auto leaf_key = bytes_view(n.get_leaf()->key());
	if (compare(leaf_key, key) >= 0)
		return const_iterator(n.get_leaf(), &root);

	/* If we arrived here, it means that the compressed bytes of leaf_key
	 * and key differ. We must descend into the tree again to the point of
	 * divergence. */
	auto diff = prefix_diff(key, leaf_key);
	auto at = static_cast<unsigned char>(leaf_key[diff] ^ key[diff]);
	auto sh = pmem::detail::mssb_index((uint32_t)at) & SLICE_MASK;
	...
	/* The searched-for element is the first element in the subtree to the
	 * right. */
	return const_iterator(next_leaf<node::direction::Forward>(
				      prev->make_iterator(child_slot), prev),
			      &root);
}
...
radix_tree<Key, Value, BytesView>::inline_string_reference<
	IsConst>::inline_string_reference(leaf_ptr leaf_,
					  node_ptr root) noexcept
