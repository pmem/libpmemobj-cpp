...
radix_tree<Key, Value, BytesView>::leaf::value() const
{
	...
	auto val_dst = reinterpret_cast<const Value *>(
		key_dst + real_bytes<Key>::value(key()));
	...
	detail::index_sequence<I2...>)
{
	...
	auto key_size = real_bytes<Key>::value(std::get<I1>(first_args)...);
	auto val_size = real_bytes<Value>::value(std::get<I2>(second_args)...);
...
struct bytes_view<obj::experimental::basic_inline_string<CharT, Traits>> {
	template <
		typename C,
...
template <typename CharT, typename Traits>
struct bytes_view<obj::basic_string<CharT, Traits>> {
	template <
		typename C,
		typename Enable = typename std::enable_if<std::is_constructible<
			obj::basic_string_view<CharT, Traits>, C>::value>::type>
