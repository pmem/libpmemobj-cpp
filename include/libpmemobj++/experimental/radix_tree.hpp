...
struct radix_tree<Key, Value, BytesView>::node {
	...
	template <bool Direction>
	typename std::enable_if<Direction == direction::Forward,
	...
	/* rbegin */
	template <bool Direction>
	typename std::enable_if<Direction == direction::Reverse,
						    ...
						    F &&make_leaf)
{
	...
	if (diff == key.size()) {
		...
		/* If path length from root to n is exactly key.size() */
		...
		/* Path length from root to n longer than key.size().
