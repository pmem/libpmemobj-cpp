...
public:
	...
	{
#if 0
		if (offset == nullptr_offset) return nullptr;
		return reinterpret_cast<byte_pointer>(
			       const_cast<this_type *>(this)) +
			offset;
#else
		static_assert(
			sizeof(uintptr_t) == sizeof(byte_pointer),
			"This method requires the size of uintptr_t to be equal to the size of the pointer");
		uintptr_t mask = offset == nullptr_offset;
		--mask;
...
protected:
	...
	{
		return reinterpret_cast<const_byte_pointer>(&ptr) -
			reinterpret_cast<const_byte_pointer>(this);
	}
	...
	pointer_to_offset(byte_pointer ptr) const noexcept
	{
		...
		return ptr - reinterpret_cast<const_byte_pointer>(this);
	}
...
private:
	static constexpr difference_type nullptr_offset = 1;
};
