...
#ifndef PMEMOBJ_SWMR_SKIP_LIST_IMPL_HPP
#define PMEMOBJ_SWMR_SKIP_LIST_IMPL_HPP
...
public:
	...
	using node_pointer =
		obj::experimental::pa_self_relative_ptr<swmr_skip_list_node>;
	using atomic_node_pointer = std::atomic<node_pointer>;
	using mutex_type = Mutex;
	using lock_type = LockType;

	swmr_skip_list_node(size_type levels) : height_(levels)
	{
	...
	swmr_skip_list_node(size_type levels, const node_pointer *new_nexts)
	    : height_(levels)
	{
		for (size_type lev = 0; lev < height_; ++lev)
			detail::create<atomic_node_pointer>(&get_next(lev),
	...
	swmr_skip_list_node(const swmr_skip_list_node &) = delete;

	swmr_skip_list_node &operator=(const swmr_skip_list_node &) = delete;
...
class swmr_skip_list_iterator {
	using node_type = NodeType;
	using node_ptr = typename std::conditional<is_const, const node_type *,
						   node_type *>::type;
	friend class swmr_skip_list_iterator<node_type, true>;
...
public:
	...
	swmr_skip_list_iterator() : node(nullptr)
	{
	}
	...
	operator++(int)
	{
		swmr_skip_list_iterator tmp = *this;
		++*this;
		return tmp;
	}

	swmr_skip_list_iterator &
	operator=(const swmr_skip_list_iterator &other)
	{
		node = other.node;
		return *this;
	}
...
private:
	explicit swmr_skip_list_iterator(node_type *n) : node(n)
	{
	}

	template <typename T = void,
		  typename = typename std::enable_if<is_const, T>::type>
	explicit swmr_skip_list_iterator(const node_type *n) : node(n)
	{
	}
	...
	friend bool operator==(const swmr_skip_list_iterator<T, M> &lhs,
			       const swmr_skip_list_iterator<T, U> &rhs);
...
template <typename T, bool M, bool U>
bool
operator==(const swmr_skip_list_iterator<T, M> &lhs,
	   const swmr_skip_list_iterator<T, U> &rhs)
{
 ...
 *
 * Each time, the pool with swmr_skip_list is being opened, the
 * swmr_skip_list requires runtime_initialize() to be called in order to
...
protected:
	...
	using list_node_type = swmr_skip_list_node<value_type>;

	using iterator = swmr_skip_list_iterator<list_node_type, false>;
	using const_iterator = swmr_skip_list_iterator<list_node_type, true>;
	...
	using persistent_node_ptr =
		obj::experimental::pa_self_relative_ptr<list_node_type>;
...
public:
	 ...
	 * contents of other.
	 *
	 * @param[in] other reference to the swmr_skip_list to be copied.
	 *
	 * @pre must be called in transaction scope.
	 *
	...
	/**
	 * Initialize swmr_skip_list after process restart.
	...
	/**
	 * Should be called before swmr_skip_list destructor is called.
	...
	/**
	 * Destructor.
	 * free_data should be called before swmr_skip_list
	 ...
	 */
	swmr_skip_list &
	operator=(const swmr_skip_list &other)
	{
		if (this == &other)
			return *this;
...
private:
	...
	void
	internal_move(swmr_skip_list &&other)
	{
		assert(this->empty());
		assert(pmemobj_tx_stage() == TX_STAGE_WORK);
	...
	void
	internal_copy(const swmr_skip_list &other)
	{
...
public:
	...
	/**
	 * pmem::detail::swmr_skip_list allows multimapping. If this flag
