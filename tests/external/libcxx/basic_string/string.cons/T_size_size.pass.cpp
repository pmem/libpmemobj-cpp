...
run(pmem::obj::pool<root> &pop)
{
...

	{
		...
		nvobj::transaction::run(pop, [&] {
			auto s = nvobj::make_persistent<S>();
			*s = "ABCD";
			(void)s;
			UT_ASSERT(false);
		});
		...
		nvobj::transaction::run(pop, [&] {
			auto s5 = nvobj::make_persistent<S>(
				arr, 0, 2); // calls ctor(const char *, len)
			UT_ASSERT(*s5 == "EFG");
		...
		nvobj::transaction::run(pop, [&] {
			auto s6 = nvobj::make_persistent<S>(
				arr, 0); // calls ctor(const char *, len)
			UT_ASSERT(*s6 == "");
		...
		nvobj::transaction::run(pop, [&] {
			auto s7 = nvobj::make_persistent<S>(
				s.data(), 2); // calls ctor(const char *, len)
			UT_ASSERT(*s7 == "");
