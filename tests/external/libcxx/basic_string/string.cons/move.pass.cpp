...
struct root {
	...
	nvobj::persistent_ptr<pmem_exp::string> s2;
};
...
test(S &s0, pmem::obj::pool<root> &pop)
{
	...
	nvobj::transaction::run(pop, [&] {
		r->s1 = nvobj::make_persistent<pmem_exp::string>(s0);
	});

	nvobj::transaction::run(pop, [&] {
		r->s2 = nvobj::make_persistent<pmem_exp::string>(std::move(s0));
	});
	...
	nvobj::transaction::run(pop, [&] {
		...
		UT_ASSERT(r->s2->c_str() ==
			  static_cast<const pmem_exp::string &>(*r->s2).data());
	});
