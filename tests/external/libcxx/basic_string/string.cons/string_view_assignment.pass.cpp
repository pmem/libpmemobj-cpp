...
struct root {
	...
	nvobj::persistent_ptr<SV> sv;
};

template <class S, class SV>
void
test(S &s1, SV &sv, pmem::obj::pool<root> &pop)
{
	typedef typename S::element_type::traits_type T;
	*s1 = *sv;
		 ...
		 "abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz"}};
	{
		for (auto &str_sv_pair : vec) {
			nvobj::transaction::run(pop, [&] {
				auto ns = nvobj::make_persistent<S>(
					str_sv_pair.first);
				(void)ns;
				auto nsv = nvobj::make_persistent<SV>(
					str_sv_pair.second);
				(void)nsv;
				test(ns, nsv, pop);
			});
