...
test(S &str, unsigned pos, pmem::obj::pool<root> &pop)
{
	...
	if (pos <= str.size()) {
		nvobj::transaction::run(pop, [&] {
			r->s1 = nvobj::make_persistent<pmem_exp::string>(str,
									 pos);
		});
		auto &s2 = *r->s1;
		...
		nvobj::transaction::run(pop, [&] {
			...
			UT_ASSERT(s2.c_str() ==
				  static_cast<const pmem_exp::string &>(s2)
					  .data());
		});
...
{
	try {
		nvobj::persistent_ptr<pmem_exp::string> s_default, s1, s2, s3;

		nvobj::transaction::run(pop, [&] {
			s_default = nvobj::make_persistent<pmem_exp::string>();
		});

		nvobj::transaction::run(pop, [&] {
			s1 = nvobj::make_persistent<pmem_exp::string>("1");
		});

		nvobj::transaction::run(pop, [&] {
			s2 = nvobj::make_persistent<pmem_exp::string>(
