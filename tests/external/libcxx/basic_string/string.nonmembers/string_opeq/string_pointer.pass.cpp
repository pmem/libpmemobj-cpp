...
run(pmem::obj::pool<root> &pop)
{
	...
	try {
		...
		test(*r->s1, "", true);
		test(*r->s1, "abcde", false);
		test(*r->s1, "abcdefghij", false);
		test(*r->s1, "abcdefghijklmnopqrst", false);
		test(*r->s2, "", false);
		test(*r->s2, "abcde", true);
		test(*r->s2, "abcdefghij", false);
		test(*r->s2, "abcdefghijklmnopqrst", false);
		test(*r->s3, "", false);
		test(*r->s3, "abcde", false);
		test(*r->s3, "abcdefghij", true);
		test(*r->s3, "abcdefghijklmnopqrst", false);
		test(*r->s4, "", false);
