...
{
	UT_ASSERT(s.find_first_of(str, pos) == x);
	if (x != S::npos)
...
{
	UT_ASSERT(s.find_first_of(str) == x);
	if (x != S::npos)
		UT_ASSERT(x < s.size());
}
...
test0(nvobj::pool<struct root> &pop)
{
	...
	test(*s_arr[71], "dfkap", 0, 1);
	test(*s_arr[7], "ihqrfebgad", 0, 2);
...
test1(nvobj::pool<struct root> &pop)
{
	...
	test(*s_arr[28], "kantesmpgj", 0);
	test(*s_arr[61], "oknlrstdpiqmjbaghcfe", 0);
	test(*s_arr[19], "", S::npos);
	test(*s_arr[57], "bnrpe", 0);
	test(*s_arr[37], "jtdaefblso", 0);
	test(*s_arr[33], "oselktgbcapndfjihrmq", 0);
	test(*s_arr[30], "", S::npos);
	test(*s_arr[72], "bjaht", 1);
	test(*s_arr[68], "hjlcmgpket", 0);
	test(*s_arr[69], "htaobedqikfplcgjsmrn", 0);
}
