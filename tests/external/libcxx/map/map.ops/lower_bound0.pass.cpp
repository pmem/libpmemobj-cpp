...
run(pmem::obj::pool<root> &pop)
{
	auto robj = pop.root();
	{
		...
		M &example = *robj->s;
		M::iterator result = example.lower_bound(C2Int{5});
	...
	}
	{
		...
		M &example = *robj->s2;
		UT_ASSERT(example.lower_bound(C2Int{5}) == example.end());
		pmem::obj::transaction::run(
	...
	}
	{
		...
		pmem::obj::transaction::run(
			pop, [&] { robj->s = nvobj::make_persistent<M>(); });
		M &example = *robj->s;
		M::const_iterator result = example.lower_bound(C2Int{5});
		UT_ASSERT(result == example.end());
		pmem::obj::transaction::run(
			pop, [&] { nvobj::delete_persistent<M>(robj->s); });
	}
...
test(int argc, char *argv[])
{
	...
	try {
		pop = pmem::obj::pool<root>::create(path, "lower_bound0.pass",
						    PMEMOBJ_MIN_POOL,
						    S_IWUSR | S_IRUSR);
	} catch (...) {
