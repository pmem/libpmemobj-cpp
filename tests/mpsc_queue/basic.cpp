...
consume_empty(pmem::obj::pool<root> pop)
{
	...
	bool consumed = queue.try_consume([&](auto rd_acc) { ASSERT_REACHED; });
...
consume_empty_after_insertion(pmem::obj::pool<root> pop)
{
	...
	for (const auto &e : values) {
		worker.try_produce(e.size(), [&](auto range) {
			std::copy_n(e.begin(), e.size(), range.begin());
		});
	}
	/* Consume all of it */
	queue.try_consume([&](auto rd_acc) {
		size_t i = 0;
...
basic_test(pmem::obj::pool<root> pop, bool create)
{
	...
	if (create) {
		...
		worker.try_produce(tmp.size(), [&](auto range) {
