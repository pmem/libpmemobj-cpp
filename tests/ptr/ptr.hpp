...
is_persistent_ptr()
{
	return std::is_same<pointer<T>, nvobj::persistent_ptr<T>>::value;
}
...
test_ptr_transactional(nvobj::pool<templated_root<pointer, pointer_base>> &pop)
{
	...
	try {
		nvobj::transaction::run(pop, [&] {
			// if needed because pool::create does not
			// call the constructor
			if (is_persistent_ptr<foo, pointer>())
	...
	try {
		nvobj::transaction::run(pop, [&] {
			...
			nvobj::delete_persistent<foo>(
				static_cast<nvobj::persistent_ptr<foo>>(
					to_swap));
		});
	} catch (...) {
		UT_ASSERT(0);
	...
	try {
		...
		if (!is_persistent_ptr<foo, pointer>())
			r->pfoo = nullptr; // self-relative-ptr hasnt
					   // specialization delete_persistent
	} catch (...) {
		UT_ASSERT(0);
	}
