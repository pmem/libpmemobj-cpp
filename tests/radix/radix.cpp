...
using container_int_int =
	nvobjex::radix_tree<unsigned, nvobj::p<int>,
			    nvobjex::big_endian_bytes_view<unsigned>>;
...
verify_elements(nvobj::persistent_ptr<Container> ptr, int count, F &&value_f)
{
	...
	std::vector<decltype(key<Container>(0))> keys;
	for (int i = 0; i < count; i++)
		keys.emplace_back(key<Container>(i));
...
test_erase(nvobj::pool<root> &pop, nvobj::persistent_ptr<Container> &ptr)
{
	...
	verify_elements(ptr, num_elements, value_f);
