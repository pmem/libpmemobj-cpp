...
test_find(nvobj::pool<root> &pop)
{
	...
	/* Test *_bound when leaf nodes' keys differ from the searched-for key
	 * at compressed bytes. */
	UT_ASSERT(r->radix_str->lower_bound("acaaay") ==
		  r->radix_str->find("acxxxy"));
	UT_ASSERT(r->radix_str->upper_bound("acaaay") ==
		  r->radix_str->find("acxxxy"));
	UT_ASSERT(r->radix_str->lower_bound("acyyyy") == r->radix_str->end());
	UT_ASSERT(r->radix_str->upper_bound("acyyyy") == r->radix_str->end());
...
test_binary_keys(nvobj::pool<root> &pop)
{
	...
	for (unsigned i = 1; i < 2 * std::numeric_limits<uint16_t>::max() - 2U;
		...
		UT_ASSERT(lit->first == i + 1);

		auto uit = r->radix_int_int->upper_bound(i);
		UT_ASSERT(uit->first == i + 1);
	}
	...
	for (unsigned i = 2 * std::numeric_limits<uint16_t>::max(); i > 0;
		...
		UT_ASSERT(it->first == i - 2);
		UT_ASSERT(it->second == i - 2);
...
test_assign_inline_string(nvobj::pool<root> &pop)
{
	...
	for (int i = 0; i < 1000; i++) {
		new_value += "x";
		r->radix_str->find("key")->second = new_value;
	}

	UT_ASSERT(nvobj::string_view(r->radix_str->find("key")->second)
			  .compare(new_value) == 0);
