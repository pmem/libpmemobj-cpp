...
static size_t INITIAL_ELEMENTS = 64;
...
void
test_write_erase_upper_lower_bounds_slit(
	nvobj::pool<root> &pop, nvobj::persistent_ptr<container_string> &ptr)
{
	const size_t value_repeats = 10;
	const size_t repeats = 100;
	...
	for (auto &k : keys_to_process) {
		 ...
		 * intern split and compress radix tree. */
		auto writer_eraser = [&] {
			for (size_t i = 0; i < repeats; i++) {
				auto ret = ptr->emplace(k + "EXTRA", k);
				UT_ASSERT(ret.second);
				UT_ASSERT(ptr->erase(k + "EXTRA") == 1);
			}
		};
...
test(int argc, char *argv[])
{
	...
	test_write_erase_upper_lower_bounds_slit(pop, pop.root()->radix_str);
